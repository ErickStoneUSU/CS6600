...
def components_5():
    ...
    conv_layer_5 = conv_2d(pool_layer_4, nb_filter=80, filter_size=3, activation='relu', name='conv_layer_5')
    pool_layer_5 = max_pool_2d(conv_layer_5, 2, name='pool_layer_5')
    fc_layer_1 = fully_connected(pool_layer_5, 100, activation='relu', name='fc_layer_1')
    fc_layer_2 = fully_connected(fc_layer_1, 10, activation='softmax', name='fc_layer_2')
    return fc_layer_2


def build_tflearn_convnet_5():
    network = regression(components_5(), optimizer='sgd', loss='categorical_crossentropy', learning_rate=0.01)
